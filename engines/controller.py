...
from engines import YandexEngine, DeepLEngine, PapagoEngine, AmazonEngine, GoogleEngine, MicrosoftEngine, \
    BaseTranslationEngine
from errors import TranslationEngineNotConfiguredError, EngineApiError, InvalidEngineNameError, \
    UnsupportedLanguagePairError, NoValidEngineConfiguredError, InvalidLanguagePreferencesError
...
def _load_language_preferences(path: Optional[str] = None) -> Tuple[LangPref, List[str]]:
    try:
    ...
    try:
        ...
        raise InvalidLanguagePreferencesError("language preferences yaml must have an xx.xx (default) ordering")

    diff = set(ENGINE_NAME_MAP.keys()) - set(ordering)
    if diff:
        raise InvalidLanguagePreferencesError(f"all engines must be included in the default ordering - missing {diff}")
...
def _best_engine_by_language(engines: Dict[str, BaseTranslationEngine], from_language: Optional[str],
                             to_language: str,
                             language_preferences: LangPref,
                             base_ordering: List[str]) -> BaseTranslationEngine:

    from_l_preferences = language_preferences.get(from_language, language_preferences["xx"])

    default_from_language_ordering = from_l_preferences.get("xx", base_ordering)
    specified_ordering = from_l_preferences.get(to_language, default_from_language_ordering)
    ...
    def get_available_engines(self):
        for engine_cls in SUPPORTED_ENGINES:
            try:
                ...
                self._logger.info("could not initialize %s engine", engine_cls.NAME, exc_info=True)
        self._logger.info("initialized the following engines %s", self.available_engines.keys())

    def get_best(self, needs_detection: bool, needs_alignment: bool, from_language: Optional[str],
                 to_language: str, exclude_engines: List[str]):
        ...
        for engine_name, engine_instance in self.available_engines.items():
            ...
            if needs_alignment and not engine_instance.supports_alignment:
                continue
            try:
                engine_instance.is_language_pair_supported(from_language=from_language, to_language=to_language)
            except UnsupportedLanguagePairError:
                continue
            engine_candidates[engine_name] = engine_instance
        if len(engine_candidates):
            return _best_engine_by_language(engine_candidates, from_language, to_language,
                                            language_preferences=default_language_preferences,
                                            base_ordering=default_ordering)
        raise NoValidEngineConfiguredError("no configured engine could carry out the request")

    def get_engine(self, name: str, needs_detection: bool, needs_alignment: bool, from_language: Optional[str],
                   to_language: str, exclude_engines: Optional[List[str]] = None):
        if name == BEST:
            return self.get_best(needs_detection, needs_alignment, from_language,
                                 to_language, exclude_engines or [])
        try:
